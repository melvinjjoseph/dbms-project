...
def due_bills():
    ...
    mycursor.execute("CREATE TABLE IF NOT EXISTS due_bills (bill_id INT NOT NULL, cust_id INT NOT NULL, meter_no INT NOT NULL, units INT NOT NULL, cost_per_unit INT NOT NULL, amount INT NOT NULL, due_date DATE NOT NULL, board_id INT NOT NULL, FOREIGN KEY (cust_id) REFERENCES customer(cust_id), FOREIGN KEY (board_id) REFERENCES Board(eb_id))")
    ...
    if not myresult:
        ...
        for x in myresult:
            ...
            due_date=x[6]
            board_id=x[7]
            sql = "INSERT INTO due_bills (bill_id, cust_id, meter_no, units, cost_per_unit, amount, due_date, board_id) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)"
            val = (bill_id, cust_id, meter_no, units, cost_per_unit, amount, due_date, board_id)
...
def update_bill():
    ...
    if not myresult:
        ...
        update_bill_db(bill_id,cust_id,meter_no,units,cost_per_unit, due_date, board_id)
...
def due_bill():
    ...
    mycursor.execute("CREATE TABLE IF NOT EXISTS due_bills (cust_id INT NOT NULL, meter_no INT NOT NULL, units INT NOT NULL, cost_per_unit INT NOT NULL, amount INT NOT NULL, due_date DATE NOT NULL, board_id INT NOT NULL, FOREIGN KEY (cust_id) REFERENCES customer(cust_id), FOREIGN KEY (board_id) REFERENCES Board(eb_id))")
    mycursor.execute("CREATE TRIGGER IF NOT EXIST due_bills_trigger AFTER UPDATE ON bill FOR EACH ROW BEGIN IF NEW.due_date < CURDATE() THEN INSERT INTO due_bills (cust_id, meter_no, units, cost_per_unit, amount, due_date, board_id) VALUES (NEW.cust_id, NEW.meter_no, NEW.units, NEW.cost_per_unit, NEW.amount, NEW.due_date, NEW.board.id); END IF; END;")
    mycursor.execute("SELECT * FROM due_bills")
    myresult = mycursor.fetchall()
    df=pd.DataFrame(myresult,columns=['cust_id','meter_no','units','cost_per_unit','amount','due_date', 'board_id'])
    st.dataframe(df)
